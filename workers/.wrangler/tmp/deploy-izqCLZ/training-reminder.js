...
// training-reminder.ts
var import_web_push = __toESM(require_src());
var FOCUS_DISPLAY = {
  s1_lun: "\u{1F3CB}\uFE0F Squat & Deadlift",
  s1_mar: "\u{1F4AA} Bench & Back",
  s1_jeu: "\u{1F4E6} Volume Haut",
  s1_ven: "\u{1F4E6} Volume Bas",
  s1_sam: "\u{1F525} Hypertrophie Haut",
  s2_lun: "\u{1F3CB}\uFE0F Squat & Deadlift",
  s2_mar: "\u{1F4AA} Bench & Back",
  s2_jeu: "\u{1F4E6} Volume Haut",
  s2_ven: "\u{1F4E6} Volume Bas",
  s2_sam: "\u{1F525} Hypertrophie Haut",
  s3_lun: "\u26A1 Puissance Bas",
  s3_mer: "\u26A1 Puissance Haut",
  s3_ven: "\u{1F525} Full Body",
  s4_lun: "\u{1F534} Lourd \u2014 Squat & DL",
  s4_mar: "\u{1F534} Lourd \u2014 Bench",
  s4_jeu: "\u26A1 Explosivit\xE9 Bas",
  s4_ven: "\u{1F4AA} Maintien Haut",
  s5_j1: "\u{1F3C6} TEST SQUAT",
  s5_j2: "\u{1F3C6} TEST BENCH",
  s5_j3: "\u{1F3C6} TEST DEADLIFT",
  s6_test_lun: "\u{1F3C6} PR Squat & DL",
  s6_test_mer: "\u{1F3C6} PR Bench",
  s6_test_ven: "\u{1F3C6} PR Deadlift",
  s6_dec_lun: "\u{1F33F} D\xE9charge Bas",
  s6_dec_mer: "\u{1F33F} D\xE9charge Haut",
  s6_dec_ven: "\u{1F33F} Mobilit\xE9 & R\xE9cup"
};
var PUSH_MESSAGES = {
  s1: [
    "Volume S1. Chaque s\xE9rie d'aujourd'hui est une fondation.",
    "80% et r\xE9gularit\xE9 \u2014 c'est ici que le cycle se construit.",
    "S1 d\xE9marre. Sois propre, sois constant.",
    "Le moteur se r\xE8gle en S1. Aujourd'hui tu poses les bases.",
    "Premi\xE8re brique du cycle. En salle."
  ],
  s2: [
    "S2 \u2014 Les habitudes se gravent. Tiens le cap.",
    "Volume maintenu. La r\xE9gularit\xE9 bat l'intensit\xE9.",
    "Deuxi\xE8me semaine \u2014 consolide ce que S1 a commenc\xE9.",
    "80% encore. La technique prime sur l'ego.",
    "S2 \u2014 C'est la constance qui construit un athl\xE8te."
  ],
  s3: [
    "S3 \u2014 L'intensit\xE9 monte, le volume recule. C'est voulu.",
    "Moins de s\xE9ries, plus de poids. Reste concentr\xE9.",
    "85-87% aujourd'hui \u2014 \xE9chauffement long obligatoire.",
    "Transmutation S3. Tu passes en mode puissance.",
    "S3 \u2014 Approche diff\xE9remment. La charge parle maintenant."
  ],
  s4: [
    "S4 \u2014 Semaine la plus lourde du cycle. Sois pr\xEAt.",
    "90%. C'est ici que le cycle se gagne ou se perd.",
    "Acclimatation S4 \u2014 \xE9chauffement long, mouvement parfait.",
  ...
  s5: [
    ...
    "Test aujourd'hui. Vise 2 \xE0 4 reps parfaites. Stop.",
    "Peaking. Tout ce que tu as fait m\xE8ne \xE0 aujourd'hui.",
    "Visualise, respire, ex\xE9cute. Tu es pr\xEAt.",
    "AMRAP de v\xE9rit\xE9. Pas d'ego \u2014 laisse le travail parler."
  ],
  s6_test: [
    "Jour J. Les maxes arrivent. Technique avant tout.",
    "6 semaines pour ce moment. Concentre-toi.",
    "PR \u2014 \xE9chauffement progressif, rien de forc\xE9.",
    "Ce n'est pas une \xE9preuve, c'est une d\xE9monstration.",
    "Confiance. Tu as fait le travail. Maintenant montre-le."
  ],
  s6_dec: [
    "D\xE9charge S6 \u2014 le corps r\xE9cup\xE8re et se renforce.",
    "50-60% aujourd'hui. Mouvement fluide, aucune douleur.",
    "Le plus dur est derri\xE8re toi. Laisse le corps absorber.",
    "D\xE9charge active \u2014 prot\xE8ge le travail accompli.",
    "Derni\xE8re semaine. Le prochain cycle sera plus fort."
  ]
};
var WEEK_SCHEDULE_MAP = {
  ...
  async scheduled(_event, env, _ctx) {
    ...
    await Promise.allSettled(
      list.keys.map(async (key) => {
        ...
        if (!data) return;
        const { subscription, weekId } = JSON.parse(data);
        const weekSchedule = WEEK_SCHEDULE_MAP[weekId] ?? {};
        const sessionId = weekSchedule[dayOfWeek];
        if (sessionId) {
          const title = FOCUS_DISPLAY[sessionId] ?? "\u26A1 S\xE9ance aujourd'hui";
          const pool = PUSH_MESSAGES[weekId] ?? PUSH_MESSAGES.s1;
          ...
          try {
            await import_web_push.default.sendNotification(subscription, payload, vapidDetails);
          } catch (err) {
            const e = err;
...
export {
  training_reminder_default as default
};
...
*/
//# sourceMappingURL=training-reminder.js.map
